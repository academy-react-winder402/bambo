...
const Bottom = ({ title, modares, zarfiat, price, set,level,type,like,dis ,id,get}) => {
    ...
    return (
        ...
        <div className=" pb-[1rem] bg-[#fff] rounded-b-lg relative">
            ...
            <div className=" h-[2rem] text-right mr-[1rem] text-sm ">    {modares} : <span> نام استاد‌ </span> </div>
            <div className=" h-[2rem] text-right mr-[1rem] text-sm ">   {level}  </div>
            <div className=" h-[2rem] text-right mr-[1rem] text-sm ">   {type}  </div>
            ...
            <button className=" h-[2rem] w-[4rem] bg-[#004458] text-white text-center rounded-md hover:cursor-pointer absolute
                 top-[10.8rem] left-[1rem]" onClick={() => { 
                    set(true);
                    get({id,title});
                  }}> جزئیات </button>


            <div className=" h-[1.5rem]  mt-[0.2rem] flex justify-end ">
                ...
                <div className=" h-[1.5rem] w-[3.5rem] flex justify-between mr-[0.8rem]">
